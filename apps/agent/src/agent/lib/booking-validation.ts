...
export const bookingInputSchema = v.object({
  ...
  preferredBookingAt: v.optional(v.string()),
...
export function missingBookingFields(
  input: Pick<BookingInput, "visitorName" | "visitorEmail" | "visitorPhone" | "matterSummary" | "companyName" | "preferredBookingAt">,
  requiredContactFields: string[],
  options?: { requirePreferredBookingAt?: boolean },
): string[] {
  ...
  if ((options?.requirePreferredBookingAt ?? true) && !input.preferredBookingAt) missing.push("preferred_booking_at");
  return missing;
}
